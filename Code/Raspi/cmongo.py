...
def capture_image(retries=3):
    ...
    Retries up to a specified number of times if the capture fails.
    :param retries: int - Number of retry attempts.
    :return: str - The filename of the saved image, or None if the capture failed.
    """
    for attempt in range(retries):
        video = cv2.VideoCapture("rtsp://admin:VYMNOK@192.168.140.38:554/H.264")
        video.set(cv2.CAP_PROP_BUFFERSIZE, 1)  # Reduce buffer size for faster reads
        time.sleep(0.5)  # Allow the connection to stabilize
        ...
        if ret:
            ...
            print(f"Image capture attempt {attempt + 1} failed.")
        video.release()
    ...
    try:
        while True:
            ...
            sensor_document = {
                "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                "moisture": {
                    "value": moisture_data[0],
                    "percentage": moisture_data[1]
                },
                "ph_sensor": {
                    "ph_value": ph_data[0],
                    "depth": ph_data[1],
