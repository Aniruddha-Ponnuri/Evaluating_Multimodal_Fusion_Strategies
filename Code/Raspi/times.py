...
def capture_image(retries=3):
    ...
    for attempt in range(retries):
        video = cv2.VideoCapture("rtsp://admin:VYMNOK@192.168.209.38:554/H.264")
...
def collect_and_store_data(n_readings):
    """
    Gathers sensor data and stores it in MongoDB only after all sensors return data.
    Takes a parameter for the number of complete reading cycles to perform.
    
    :param n_readings: int - The number of sensor reading cycles to collect.
    """
    try:
        for cycle in range(n_readings):
            print(f"\nStarting data collection cycle {cycle + 1} of {n_readings}")
            moisture_data = None
            ph_data = None
            npk_data = None
            image_filename = None
            ...
            while moisture_data is None or ph_data is None or npk_data is None or image_filename is None:
                ...
                if image_filename is None:
                    print("Skipping image capture for this cycle temporarily. Retrying...")
...
if __name__ == "__main__":
    ...
    collect_and_store_data(n_readings)
