...
def read_mcp3008(channel=0):
    ...
    spi.mode = 0  # Set SPI mode to 0 (MCP3008 default)

    try:
        # MCP3008 communication to read data
        adc = spi.xfer2([1, (8 + channel) << 4, 0])
        print(f"SPI Raw Response: {adc}")  # Debug print

        # Combine the two bytes to form the 10-bit ADC value
        raw_value = ((adc[1] & 3) << 8) + adc[2]

        # Map raw value to percentage safely
        percentage = int((raw_value / 1023.0) * 100) if raw_value else 0
...
if __name__ == "__main__":
    try:
        ...
        while True:
            raw, percent = read_mcp3008(channel=0)
